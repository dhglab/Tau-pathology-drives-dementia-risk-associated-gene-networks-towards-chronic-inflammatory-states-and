...
# load normalized TPR50 cortex RNAseq data (outliers removed) (published - Swarup et al, 2019)

load("../data_provided/Data_6M_Ctx.rda")

# read expression data

datExpr_C57BL6=as.data.frame(datExpr[targets$Strain=="C57"|targets$Strain=="C57BL6",])

datExpr_DBA =as.data.frame(datExpr[targets$Strain=="DBA",])#11 samples

datExpr_FVB =as.data.frame(datExpr[targets$Strain=="FVB",])#9 samples
...
# load cleaned and normalized Tg4510 microglia RNAseq data (published - Wang et al., 2018)

load("micro_Wang.rda")

#read and format expression data and metadata for WGCNA

micro = read.table("microglia_normalized_logsignal.txt", header = TRUE)

meta = read.table("meta_data.txt", sep = "\t", header = TRUE)

targets_micro = meta

datExpr_micro = t(normExpr_micro)

rownames(datExpr_micro) = colnames(normExpr_micro)

colnames(datExpr_micro) = rownames(normExpr_micro)
...
multiMeta=list(Tg4510_microglia =list(data=targets_micro),TPR50.DBA=list(data=targets_DBA),TPR50.FVB=list(data=targets_FVB),TPR50.C57=list(data=targets_C57BL6))

checkSets(multiMeta) # check data content

save(list=ls(),file="Data_forConsensusWGCNA_formatted.rda")
...
###### combine data	and plot final dendrogram
		...
		save(mColorh,tmpMulti[[1]]$genecols,tmpMulti[[2]]$genecols,tmpMulti[[3]]$genecols,tmpMulti[[4]]$genecols, file = "combo.rda")
		
		
		  mColorh1 <- cbind(mColorh,t(tmpMulti[[1]]$genecols),t(tmpMulti[[2]]$genecols),t(tmpMulti[[3]]$genecols),t(tmpMulti[[4]]$genecols))
		  
		  mLabelh1 <- c(mLabelh,rownames(tmpMulti[[1]]$genecols),rownames(tmpMulti[[2]]$genecols),rownames(tmpMulti[[3]]$genecols),rownames(tmpMulti[[4]]$genecols))
...
tree = cutreeHybrid(dendro = consTree, pamStage=FALSE,
         ...
         merged <- mergeCloseModules(exprData = multiExpr,colors = tree$labels,
                                        cutHeight = dthresh)     
        mColorh <- cbind(labels2colors(merged$colors),t(multiData.Tg4510_microglia$genecols), t(multiData.TPR50_DBA$genecols),t(multiData.TPR50_FVB$genecols),t(multiData.TPR50_C57$genecols))
        
        mLabelh <- c("Merged Colors",rownames(multiData.Tg4510_microglia$genecols),rownames(multiData.TPR50_FVB$genecols),rownames(multiData.TPR50_DBA$genecols),rownames(multiData.TPR50_C57$genecols))
  ...
  plotDendroAndColors(consTree, mColorh, groupLabels = mLabelh,addGuide=TRUE,dendroLabels=FALSE,main= paste("Signed bicor network with power =30, mms=",mms,"ds=",ds,"dthresh=",dthresh));
...
# calculate consensus kME for each gene


moduleColors_Cons = labels2colors(merged$colors)


kMEs <- consensusKME(multiExpr, moduleColors_Cons,consensusQuantile=0.20,signed=TRUE)

cons.kMEind=which(regexpr("consensus.",colnames(kMEs))>0)  

cons.kMEs=kMEs[, cons.kMEind]

rownames(cons.kMEs)=colnames(multiExpr[[1]]$data)
...
# remove genes with negative bicor 
